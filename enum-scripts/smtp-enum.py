...
class SMTPUserEnumerator():
    def __init__(self, target, userlist, port=25, scantype="vrfy",
                 mailfrom="root"):
        ...
        # boolean used for one-time sending of MAIL FROM command for RCPT scan
        self.mailfrom = [mailfrom, True]
        # attribute for socket connected to target, generate with buildSock()
        # attribute for socket connected to target, generate with buildSock()
        self.sock = None
        # banner from target server, stored upon vuln test
        self.targetBanner = None

    def readUsers(self):
        # func to read file of usernames
...
if __name__ == "__main__":
	...
	print("[*] Checking for vulnerability to %s scan... " %(scantype.upper()), end=' ');sys.stdout.flush() # begin vuln check
	try:
	...
	print("[*] Parsing list of users... ", end=' ');sys.stdout.flush() # tell user that file is being read
	try:
